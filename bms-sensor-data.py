...
if __name__ == "__main__": 
    ...
    try: 
        while True: 
            # 날짜에 따라 CSV 파일 경로 설정 (매일 새로운 파일 생성) 
            today = datetime.now().strftime('%Y-%m-%d') 
            csv_file = f'/home/pi/work_folder/bms_data/bms_data_log_{today}.csv' 
             
            # 하루마다 CSV 파일에 헤더 추가 (파일이 없을 경우) 
